...
import { syncMarketBreadthBackfill, syncMarketBreadthDaily } from "@/lib/syncMarketBreadth";
...
export async function POST(req: Request) {
  ...
  try {
    const body = (await req.json().catch(() => ({}))) as { lastDays?: number; market?: Market; backfill?: boolean };
    ...
    const result = body.backfill ? await syncMarketBreadthBackfill(lastDays, market) : await syncMarketBreadthDaily(lastDays, market);
